<div class="space-y-4">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div
            ...
            class="group relative overflow-hidden rounded-2xl border border-white/30 bg-gradient-to-br from-indigo-400/90 to-indigo-600/90 p-5 text-white shadow-lg shadow-indigo-900/20 backdrop-blur-xl transition-all duration-300 ease-spring hover:-translate-y-2 hover:scale-[1.02] hover:shadow-2xl"
        >
            <x-heroicon-o-arrow-path class="pointer-events-none absolute -right-4 -top-4 h-28 w-28 text-white/20 transition-transform duration-500 ease-spring group-hover:scale-125 group-hover:rotate-12" />
            <p class="text-3xl font-bold">{{ number_format($totalChanges) }}</p>
            <p class="mt-1 text-sm text-indigo-50">Total Perubahan</p>
        </div>
        ...
        >
            ...
            <p class="text-3xl font-bold">{{ number_format($highRiskChanges) }}</p>
            <p class="mt-1 text-sm text-orange-50">Risiko Inheren Tinggi</p>
        </div>
    </div>
    ...
    @if ($recentActivity->isNotEmpty())
        <div class="glass-panel rounded-2xl shadow-lg">
            <div class="border-b border-white/40 px-5 py-3">
                <h2 class="text-sm font-semibold text-gray-900">Aktivitas Perubahan Terbaru</h2>
            </div>
