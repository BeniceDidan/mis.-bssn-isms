    ...
    <div class="glass-panel flex flex-col gap-3 rounded-2xl p-4 shadow-lg md:flex-row md:flex-wrap md:items-center md:justify-center md:gap-6">
        <div class="relative w-full md:max-w-sm" x-data="{ focused: false }">
            ...
            <input
                ...
                placeholder="Cari kode, judul, penanggung jawab..."
        ...
        <div class="flex flex-wrap items-center gap-2">
            <select wire:model.live="typeFilter" class="rounded-full border-white/60 bg-white/50 text-sm transition-all duration-300 ease-spring hover:bg-white/80 focus:border-sky-400 focus:ring-sky-400">
                ...
                <thead class="sticky top-0 z-10 bg-white/70 backdrop-blur-xl">
                    <tr>
                        @foreach ([
                            'title' => 'Judul',
                            'change_type' => 'Jenis',
                            'inherent_risk_level' => 'Risiko Inheren',
                            'priority' => 'Prioritas',
                            'status' => 'Status',
                        ] as $field => $label)
                            <th
                        ...
                        @endforeach
                        <th class="whitespace-nowrap px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Aset Terkait</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">Aksi</th>
                    </tr>
                </thead>
                <tbody wire:loading wire:target="search,typeFilter,riskLevelFilter,showArchived,sortBy,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    <x-table-skeleton :columns="7" />
                </tbody>
                <tbody wire:loading.remove wire:target="search,typeFilter,riskLevelFilter,showArchived,sortBy,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    @forelse ($changes as $change)
                        <tr
                            wire:key="change-{{ $change->id }}"
                            class="transition-all duration-200 hover:bg-white/60 hover:shadow-sm {{ $change->trashed() ? 'opacity-60' : '' }}"
                            style="border-left: 3px solid {{ $change->inherent_risk_level?->accentHex() ?? 'transparent' }}"
                        >
                            <td class="whitespace-nowrap px-4 py-3 font-semibold text-gray-900">{{ $change->title }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $change->change_type?->label() }}</td>
                            <td class="whitespace-nowrap px-4 py-3">
                                @if ($change->inherent_risk_level)
                                    @php
                                        $isSevere = $change->inherent_risk_level->isSevere();
                                        $badgeClasses = $isSevere
                                            ...
                                            ][$change->inherent_risk_level->badgeColor()]
                                            : [
                                    ...
                                    <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-semibold transition-transform duration-300 ease-spring hover:scale-110 {{ $badgeClasses }}">
                                        {{ $change->inherent_risk_level->label() }}
                                    </span>
                                @else
                                    &mdash;
                                @endif
                            </td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $change->priority ?: '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">
                                ...
                                @php
                                    ...
                                    ][$change->verification_status] ?? null;
                                @endphp
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button wire:click="viewDetail({{ $change->id }})" title="Lihat Detail" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-sky-100 hover:text-sky-600 active:scale-90">
                                    ...
                                    @if (auth()->user()?->canWrite())
                                        @if (! $change->trashed())
                                            <button
                                                wire:click="$dispatch('open-change-form', { changeId: {{ $change->id }} })"
                                                title="Ubah"
                                            ...
                                            <button
                                                x-on:click="if (await $store.confirm.ask('Arsipkan perubahan ini?', 'Data tidak akan dihapus dan dapat dipulihkan kembali.', 'danger')) $wire.archive({{ $change->id }})"
                                                title="Arsipkan"
                                        ...
                                        @else
                                            <button wire:click="restore({{ $change->id }})" title="Pulihkan" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-emerald-100 hover:text-emerald-600 active:scale-90">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-10 text-center text-sm text-gray-500">
                                Tidak ada perubahan yang cocok dengan pencarian ini.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            >
                @if ($viewingChange)
                    <div class="flex items-center justify-between border-b border-white/40 px-5 py-4">
                        <div>
                            <p class="text-xs font-mono text-gray-400">{{ $viewingChange->change_code }}</p>
                            <h2 class="text-lg font-semibold text-gray-900">{{ $viewingChange->title }}</h2>
                        </div>
                    ...
                    <dl class="divide-y divide-white/40 px-5">
                        @foreach ([
                            'Aset Terkait' => $viewingChange->asset?->name,
                            'Jenis Perubahan' => $viewingChange->change_type?->label(),
                            'Kategori' => $viewingChange->category,
                            'Prioritas' => $viewingChange->priority,
                            'Level Risiko Inheren' => $viewingChange->inherent_risk_level?->label(),
                            'Keputusan' => $viewingChange->decision,
                            'Status' => $viewingChange->status,
                            'Penanggung Jawab' => $viewingChange->pic,
                            'Target Implementasi' => optional($viewingChange->target_date)->format('d M Y'),
                            'Kode Legacy' => $viewingChange->legacy_code,
                        ] as $label => $value)
                    ...
                    @if (! empty($viewingChange->dynamic_data))
                        <div class="border-t border-white/40 px-5 py-4">
                            <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Data Dinamis (Penilaian Risiko Lengkap)</p>
                            <dl class="divide-y divide-white/40">
                                @foreach ($viewingChange->dynamic_data as $key => $value)
                                    <div class="grid grid-cols-2 gap-2 py-2.5 text-sm">
