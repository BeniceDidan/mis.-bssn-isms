    ...
    <div class="glass-panel flex flex-col gap-3 rounded-2xl p-4 shadow-lg md:flex-row md:flex-wrap md:items-center md:justify-center md:gap-6">
        <div class="relative w-full md:max-w-sm" x-data="{ focused: false }">
            ...
            <input
                ...
                placeholder="Cari kode, judul, pemilik risiko..."
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <select wire:model.live="statusFilter" class="rounded-full border-white/60 bg-white/50 text-sm transition-all duration-300 ease-spring hover:bg-white/80 focus:border-sky-400 focus:ring-sky-400">
    ...
    <div class="glass-panel overflow-hidden rounded-2xl shadow-lg">
        <div class="max-h-[65vh] overflow-auto">
            <table class="min-w-full divide-y divide-white/40 text-sm">
                ...
                <tbody wire:loading wire:target="search,categoryFilter,riskLevelFilter,statusFilter,showArchived,sortBy,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    <x-table-skeleton :columns="6" />
                </tbody>
                <tbody wire:loading.remove wire:target="search,categoryFilter,riskLevelFilter,statusFilter,showArchived,sortBy,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    @forelse ($risks as $risk)
                        <tr
                            wire:key="risk-{{ $risk->id }}"
                            class="transition-all duration-200 hover:bg-white/60 hover:shadow-sm {{ $risk->trashed() ? 'opacity-60' : '' }}"
                            style="border-left: 3px solid {{ $risk->risk_level?->accentHex() ?? 'transparent' }}"
                        >
                            <td class="whitespace-nowrap px-4 py-3 font-semibold text-gray-900">{{ $risk->title }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $risk->category?->label() }}</td>
                            <td class="whitespace-nowrap px-4 py-3">
                                @if ($risk->risk_level)
                                    @php
                                        $isSevere = $risk->risk_level->isSevere();
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button wire:click="viewDetail({{ $risk->id }})" title="Lihat Detail" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-sky-100 hover:text-sky-600 active:scale-90">
                                    ...
                                    @if (auth()->user()?->canWrite())
                                        @if (! $risk->trashed())
                                            <button
                                                wire:click="$dispatch('open-risk-form', { riskId: {{ $risk->id }} })"
                                                title="Ubah"
                                            ...
                                            <button
                                                x-on:click="if (await $store.confirm.ask('Arsipkan risiko ini?', 'Data tidak akan dihapus dan dapat dipulihkan kembali.', 'danger')) $wire.archive({{ $risk->id }})"
                                                title="Arsipkan"
                                        ...
                                        @else
                                            <button wire:click="restore({{ $risk->id }})" title="Pulihkan" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-emerald-100 hover:text-emerald-600 active:scale-90">
            ...
            >
                @if ($viewingRisk)
                    <div class="flex items-center justify-between border-b border-white/40 px-5 py-4">
                        <div>
                            <p class="text-xs font-mono text-gray-400">{{ $viewingRisk->risk_code }}</p>
                            <h2 class="text-lg font-semibold text-gray-900">{{ $viewingRisk->title }}</h2>
                        </div>
                    ...
                    <dl class="divide-y divide-white/40 px-5">
                        @foreach ([
                            'Aset Terkait' => $viewingRisk->asset?->name,
                            'Kategori' => $viewingRisk->category?->label(),
                            'Sumber Ancaman' => $viewingRisk->threat_source,
                            'Kerentanan' => $viewingRisk->vulnerability,
                            'Kemungkinan' => $viewingRisk->likelihood?->label(),
                            'Dampak' => $viewingRisk->impact?->label(),
                            'Level Risiko' => $viewingRisk->risk_level?->label(),
                            'Pemilik Risiko' => $viewingRisk->risk_owner,
                            'Strategi Penanganan' => $viewingRisk->treatment_strategy?->label(),
                            'Status' => $viewingRisk->status?->label(),
                            'Tanggal Identifikasi' => optional($viewingRisk->identified_at)->format('d M Y'),
                            'Tinjauan Berikutnya' => optional($viewingRisk->review_due_at)->format('d M Y'),
                        ] as $label => $value)
                    ...
                    @if (! empty($viewingRisk->dynamic_data))
                        <div class="border-t border-white/40 px-5 py-4">
                            ...
                            <dl class="divide-y divide-white/40">
                                @foreach ($viewingRisk->dynamic_data as $key => $value)
                                    <div class="grid grid-cols-2 gap-2 py-2.5 text-sm">
