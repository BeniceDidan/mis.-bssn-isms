<div class="space-y-4">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div
            ...
            class="group relative overflow-hidden rounded-2xl border border-white/30 bg-gradient-to-br from-slate-400/90 to-slate-600/90 p-5 text-white shadow-lg shadow-slate-900/20 backdrop-blur-xl transition-all duration-300 ease-spring hover:-translate-y-2 hover:scale-[1.02] hover:shadow-2xl"
        >
            <x-heroicon-o-users class="pointer-events-none absolute -right-4 -top-4 h-28 w-28 text-white/20 transition-transform duration-500 ease-spring group-hover:scale-125 group-hover:rotate-12" />
            <p class="text-3xl font-bold">{{ number_format($totalRecords) }}</p>
            <p class="mt-1 text-sm text-slate-50">Total Risiko SDM</p>
        </div>
        ...
        >
            ...
            <p class="text-3xl font-bold">{{ number_format($highRiskRecords) }}</p>
            <p class="mt-1 text-sm text-orange-50">Risiko Tinggi/Kritis</p>
        </div>
    </div>
                ...
                @foreach ($recentActivity as $activity)
                    ...
                    <div class="flex items-center justify-between px-5 py-2.5 text-sm transition-all duration-200 hover:translate-x-1 hover:bg-white/50">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-gray-700">{{ $activity->loggable?->subject ?? "#{$activity->loggable_id}" }}</span>
                        </div>
