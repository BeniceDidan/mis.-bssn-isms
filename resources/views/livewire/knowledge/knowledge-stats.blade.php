<div class="space-y-4">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        >
            <x-heroicon-o-light-bulb class="pointer-events-none absolute -right-4 -top-4 h-28 w-28 text-white/20 transition-transform duration-500 ease-spring group-hover:scale-125 group-hover:rotate-12" />
            <p class="text-3xl font-bold">{{ number_format($totalRecords) }}</p>
            <p class="mt-1 text-sm text-amber-50">Total Aset Pengetahuan</p>
        </div>
        ...
        >
            <x-heroicon-o-link class="pointer-events-none absolute -right-4 -top-4 h-28 w-28 text-white/20 transition-transform duration-500 ease-spring group-hover:scale-125 group-hover:rotate-12" />
            <p class="text-3xl font-bold">{{ number_format($linkedToAsset) }}</p>
            <p class="mt-1 text-sm text-teal-50">Tertaut ke Aset</p>
        </div>
    </div>
                ...
                @foreach ($recentActivity as $activity)
                    ...
                    <div class="flex items-center justify-between px-5 py-2.5 text-sm transition-all duration-200 hover:translate-x-1 hover:bg-white/50">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-gray-700">{{ $activity->loggable?->title ?? "#{$activity->loggable_id}" }}</span>
                        </div>
