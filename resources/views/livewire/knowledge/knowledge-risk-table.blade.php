<div class="space-y-4" x-data="{ toast: null }" x-on:toast.window="toast = $event.detail.message; setTimeout(() => toast = null, 3000)">
    ...
    <div class="glass-panel flex flex-col gap-3 rounded-2xl p-4 shadow-lg md:flex-row md:flex-wrap md:items-center md:justify-center md:gap-6">
        <div class="relative w-full md:max-w-sm" x-data="{ focused: false }">
            ...
            <input type="search" wire:model.live.debounce.300ms="search" placeholder="Cari risiko, area dampak..." class="w-full rounded-full border-white/60 bg-white/50 pl-9 text-sm transition-all duration-300 ease-spring focus:scale-[1.01] focus:border-amber-400 focus:bg-white/90 focus:ring-amber-400" @focus="focused = true" @blur="focused = false">
        </div>
        <div class="flex flex-wrap items-center gap-2">
            <select wire:model.live="levelFilter" class="rounded-full border-white/60 bg-white/50 text-sm transition-all duration-300 ease-spring hover:bg-white/80 focus:border-amber-400 focus:ring-amber-400">
        ...
        <div class="max-h-[60vh] overflow-auto">
            <table class="min-w-full divide-y divide-white/40 text-sm">
                <thead class="sticky top-0 z-10 bg-white/70 backdrop-blur-xl">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Pernyataan Risiko</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Aset Terkait</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Skor</th>
                ...
                <tbody wire:loading wire:target="search,levelFilter,showArchived,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    <x-table-skeleton :columns="5" />
                </tbody>
                <tbody wire:loading.remove wire:target="search,levelFilter,showArchived,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    @forelse ($records as $record)
                        <tr wire:key="krisk-{{ $record->id }}" class="transition-all duration-200 hover:bg-white/60 hover:shadow-sm {{ $record->trashed() ? 'opacity-60' : '' }}" style="border-left: 3px solid {{ $record->tingkat_risiko_bawaan?->accentHex() ?? 'transparent' }}">
                            <td class="max-w-xs truncate px-4 py-3 font-semibold text-gray-900">
                                {{ $record->pernyataan_risiko }}
                                @php
                                    $verificationBadge = [
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $record->knowledgeAsset?->legacy_code ?? '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $record->skor_risiko_bawaan ?? '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3">
                                @if ($record->tingkat_risiko_bawaan)
                                    <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-semibold text-white" style="background-color: {{ $record->tingkat_risiko_bawaan->accentHex() }}">{{ $record->tingkat_risiko_bawaan->label() }}</span>
                                @else
                                    &mdash;
                                @endif
                            </td>
                            <td class="whitespace-nowrap px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    @if (auth()->user()?->canWrite())
                                        @if (! $record->trashed())
                                            <button wire:click="$dispatch('open-knowledge-risk-form', { recordId: {{ $record->id }} })" title="Ubah" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-amber-100 hover:text-amber-600 active:scale-90">
                                                <x-heroicon-o-pencil-square class="h-4 w-4" />
                                            </button>
                                            <button x-on:click="if (await $store.confirm.ask('Arsipkan risiko ini?', 'Data tidak akan dihapus dan dapat dipulihkan kembali.', 'danger')) $wire.archive({{ $record->id }})" title="Arsipkan" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-red-100 hover:text-red-600 active:scale-90">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-4 py-10 text-center text-sm text-gray-500">Tidak ada risiko yang cocok dengan pencarian ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
                ...
                @if ($viewingRecord)
                    <div class="flex items-center justify-between border-b border-white/40 px-5 py-4">
                        <div>
                            ...
                            <h2 class="text-lg font-semibold text-gray-900">{{ \Illuminate\Support\Str::limit($viewingRecord->pernyataan_risiko, 60) }}</h2>
                        </div>
                    ...
                    <dl class="divide-y divide-white/40 px-5">
                        @foreach ([
                            'Aset Terkait' => $viewingRecord->knowledgeAsset ? "{$viewingRecord->knowledgeAsset->legacy_code} — {$viewingRecord->knowledgeAsset->title}" : null,
                            'Akar Penyebab' => $viewingRecord->akar_penyebab,
                            'Area Dampak' => $viewingRecord->area_dampak,
                            'Dampak' => $viewingRecord->dampak,
                            'Kemungkinan' => $viewingRecord->kemungkinan,
                            'Skor Risiko Bawaan' => $viewingRecord->skor_risiko_bawaan,
                            'Tingkat Risiko Bawaan' => $viewingRecord->tingkat_risiko_bawaan?->label(),
                            'Rencana Mitigasi' => $viewingRecord->rencana_mitigasi,
                            'Penanggung Jawab' => $viewingRecord->penanggung_jawab,
                            'Tingkat Residual Risk' => $viewingRecord->tingkat_residual_risk?->label(),
