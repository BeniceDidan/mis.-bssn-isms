        ...
        <div class="max-h-[60vh] overflow-auto">
            <table class="min-w-full divide-y divide-white/40 text-sm">
                <thead class="sticky top-0 z-10 bg-white/70 backdrop-blur-xl">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Nama Kegiatan</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Tanggal</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Narasumber</th>
                ...
                <tbody wire:loading.remove wire:target="search,showArchived,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    @forelse ($records as $record)
                        <tr wire:key="activity-{{ $record->id }}" class="transition-all duration-200 hover:bg-white/60 hover:shadow-sm {{ $record->trashed() ? 'opacity-60' : '' }}">
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ optional($record->tanggal_pelaksanaan)->format('d M Y') ?: '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $record->narasumber_name ?: '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $record->jumlah_peserta ?? '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    @if (auth()->user()?->canWrite())
                                        @if (! $record->trashed())
                                            <button wire:click="$dispatch('open-knowledge-activity-form', { recordId: {{ $record->id }} })" title="Ubah" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-amber-100 hover:text-amber-600 active:scale-90">
                                                <x-heroicon-o-pencil-square class="h-4 w-4" />
                                            </button>
                                            <button x-on:click="if (await $store.confirm.ask('Arsipkan kegiatan ini?', 'Data tidak akan dihapus dan dapat dipulihkan kembali.', 'danger')) $wire.archive({{ $record->id }})" title="Arsipkan" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-red-100 hover:text-red-600 active:scale-90">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-4 py-10 text-center text-sm text-gray-500">Tidak ada kegiatan yang cocok dengan pencarian ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
                ...
                @if ($viewingRecord)
                    <div class="flex items-center justify-between border-b border-white/40 px-5 py-4">
                        <div>
                            ...
                            <h2 class="text-lg font-semibold text-gray-900">{{ $viewingRecord->nama_kegiatan }}</h2>
                        </div>
                    ...
                    <dl class="divide-y divide-white/40 px-5">
                        @foreach ([
                            'Tanggal Pelaksanaan' => optional($viewingRecord->tanggal_pelaksanaan)->format('d M Y'),
                            'Materi / Topik' => $viewingRecord->materi_topik,
                            'Narasumber' => $viewingRecord->narasumber_name,
                            'Jumlah Peserta' => $viewingRecord->jumlah_peserta,
                    ...
                    @if ($viewingRecord->link_bukti)
                        <div class="border-t border-white/40 px-5 py-4">
                            <a href="{{ $viewingRecord->link_bukti }}" target="_blank" class="flex items-center gap-2 rounded-lg border border-white/50 bg-white/50 px-3 py-2 text-sm text-sky-700 transition-all duration-200 hover:translate-x-1 hover:bg-sky-50">
                                <x-heroicon-o-link class="h-4 w-4 shrink-0" />
                                <span class="truncate">{{ $viewingRecord->link_bukti }}</span>
                            </a>
                        </div>
                    @endif
