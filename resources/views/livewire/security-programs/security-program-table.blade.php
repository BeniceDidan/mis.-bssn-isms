<div class="space-y-4" x-data="{ toast: null }" x-on:toast.window="toast = $event.detail.message; setTimeout(() => toast = null, 3000)">
    ...
    <div class="glass-panel flex flex-col gap-3 rounded-2xl p-4 shadow-lg md:flex-row md:flex-wrap md:items-center md:justify-center md:gap-6">
        <div class="relative w-full md:max-w-sm" x-data="{ focused: false }">
            <x-heroicon-o-magnifying-glass class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 transition-all duration-300 ease-spring" x-bind:class="focused ? 'text-emerald-500 scale-125' : ''" />
            <input type="search" wire:model.live.debounce.300ms="search" placeholder="Cari program kerja, kegiatan, PIC..." class="w-full rounded-full border-white/60 bg-white/50 pl-9 text-sm transition-all duration-300 ease-spring focus:scale-[1.01] focus:border-emerald-400 focus:bg-white/90 focus:ring-emerald-400" @focus="focused = true" @blur="focused = false">
        </div>
        <label class="flex cursor-pointer items-center gap-2 text-sm text-gray-600 transition-colors duration-150 hover:text-gray-800">
            <input type="checkbox" wire:model.live="showArchived" class="rounded border-gray-300 text-emerald-600 transition-colors duration-150 focus:ring-emerald-500">
            Tampilkan Arsip
        </label>
    </div>
            ...
            <table class="min-w-full divide-y divide-white/40 text-sm">
                <thead class="sticky top-0 z-10 bg-white/70 backdrop-blur-xl">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Program Kerja</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Kegiatan</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">PIC</th>
                ...
                <tbody wire:loading.remove wire:target="search,showArchived,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    @forelse ($records as $record)
                        <tr wire:key="kin-{{ $record->id }}" class="transition-all duration-200 hover:bg-white/60 hover:shadow-sm {{ $record->trashed() ? 'opacity-60' : '' }}">
                            <td class="whitespace-nowrap px-4 py-3 font-mono text-xs text-gray-700">{{ $record->legacy_code }}</td>
                            <td class="max-w-md px-4 py-3 text-gray-900">
                                {{ \Illuminate\Support\Str::limit($record->program_kerja, 90) }}
                                @php
                                    $verificationBadge = [
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $record->kegiatan ?: '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $record->pic ?: '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $record->asset?->name ?: '—' }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button wire:click="viewDetail({{ $record->id }})" title="Lihat Detail" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-emerald-100 hover:text-emerald-600 active:scale-90">
                                        <x-heroicon-o-eye class="h-4 w-4" />
                                    </button>
                                    @if (auth()->user()?->canWrite())
                                        @if (! $record->trashed())
                                            <button wire:click="$dispatch('open-security-program-form', { recordId: {{ $record->id }} })" title="Ubah" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-emerald-100 hover:text-emerald-600 active:scale-90">
                                                <x-heroicon-o-pencil-square class="h-4 w-4" />
                                            </button>
                                            <button x-on:click="if (await $store.confirm.ask('Arsipkan program kerja ini?', 'Data tidak akan dihapus dan dapat dipulihkan kembali.', 'danger')) $wire.archive({{ $record->id }})" title="Arsipkan" class="rounded-full p-1.5 text-gray-500 transition-all duration-300 ease-spring hover:scale-125 hover:bg-red-100 hover:text-red-600 active:scale-90">
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-10 text-center text-sm text-gray-500">Tidak ada program kerja yang cocok dengan pencarian ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
            ...
            <div x-show="open" x-transition:enter="transition ease-spring duration-500" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" class="glass-panel h-full w-full max-w-md overflow-y-auto shadow-2xl">
                @if ($viewingRecord)
                    <div class="flex items-center justify-between border-b border-white/40 px-5 py-4">
                        <div>
                            <p class="text-xs font-mono text-gray-400">{{ $viewingRecord->record_code }} &middot; No {{ $viewingRecord->legacy_code }}</p>
                            <h2 class="text-lg font-semibold text-gray-900">{{ \Illuminate\Support\Str::limit($viewingRecord->program_kerja, 50) }}</h2>
                        </div>
                    ...
                    <dl class="divide-y divide-white/40 px-5">
                        @foreach ([
                            ...
                            'Program Kerja' => $viewingRecord->program_kerja,
                            'Kegiatan' => $viewingRecord->kegiatan,
                            'PIC' => $viewingRecord->pic,
                        ] as $label => $value)
                    ...
                    @if (! empty($viewingRecord->dynamic_data))
                        <div class="border-t border-white/40 px-5 py-4">
                            <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Target & Realisasi Tahunan</p>
