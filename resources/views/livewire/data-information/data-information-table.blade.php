            ...
            <table class="min-w-full divide-y divide-white/40 text-sm">
                ...
                <tbody wire:loading.remove wire:target="search,typeFilter,riskLevelFilter,showArchived,sortBy,previousPage,nextPage,gotoPage" class="divide-y divide-white/40">
                    @forelse ($records as $record)
                        <tr
                            wire:key="data-information-{{ $record->id }}"
                        ...
                        >
                            <td class="whitespace-nowrap px-4 py-3 font-semibold text-gray-900">{{ $record->title }}</td>
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600">{{ $record->priority ?: '—' }}</td>
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    @if (auth()->user()?->canWrite())
                                        @if (! $record->trashed())
                                            <button
                                                wire:click="$dispatch('open-data-information-form', { recordId: {{ $record->id }} })"
                                                title="Ubah"
                                            ...
                                            <button
                                                x-on:click="if (await $store.confirm.ask('Arsipkan catatan ini?', 'Data tidak akan dihapus dan dapat dipulihkan kembali.', 'danger')) $wire.archive({{ $record->id }})"
                                                title="Arsipkan"
            ...
            >
                @if ($viewingRecord)
                    ...
                    <dl class="divide-y divide-white/40 px-5">
                        @foreach ([
                            'Aset Terkait' => $viewingRecord->asset?->name,
                            'Jenis Risiko' => $viewingRecord->risk_type?->label(),
                            'Kategori Data' => $viewingRecord->category,
                            'Prioritas' => $viewingRecord->priority,
                            'Level Risiko Inheren' => $viewingRecord->inherent_risk_level?->label(),
                            'Keputusan' => $viewingRecord->decision,
                            'Status' => $viewingRecord->status,
                            'Penanggung Jawab' => $viewingRecord->pic,
                            'Target Implementasi' => optional($viewingRecord->target_date)->format('d M Y'),
